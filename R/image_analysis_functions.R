...
plotExpression <- function(raster_obj, sce_cellLabel, exprsMat,
    ...
    ddf <- raster::rasterToPoints(raster_obj)
    ddf <- data.frame(ddf)
    colnames(ddf) <- c("X", "Y")
    ...
    new_values <- plyr::mapvalues((cell_label), 
                                  from = notInLabel, 
                                  to = rep(min(exprsMat[gene, ]),
                                           length(notInLabel)))
    
    
    new_values <- plyr::mapvalues((new_values), 
                                  from = sce_cellLabel, 
                                  to = exprsMat[gene, ])
    ...
    g <- ggplot2::ggplot(NULL) + 
        ggplot2::geom_raster(data = ddf, aes(X, Y, fill = new_values)) +
